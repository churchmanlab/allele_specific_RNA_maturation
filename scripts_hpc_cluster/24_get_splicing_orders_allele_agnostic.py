...
This script will identify possible splicing orders and assign them a score based on the partially spliced isoform counts (allele-agnostic)
...
# Identify introns that are spliced post-transcriptionally in a minimum of reads
min_reads = 10
...
chr1_splice_info = pd.read_table(sys.argv[4])
...
# Merge the introns from the three replicates
gr_per_intron_NO = chr1_gr_per_intron_NO.copy()
...
for transcript in list(transcript_dict.keys()):
    ...
    print(multi_introns_df1.columns)
    ...
    for i in range(3,4):
        for x in range(total_introns_of_interest-i+1):
            ...
            results_df = get_score_per_path_non_consec(multi_introns_df1, i, introns_of_interest_sub, n_introns_gene, strand)
    
            if results_df is not None:
                results_df['sample_name'] = s1
